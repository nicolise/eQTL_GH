...
for (j in c("col0","coi1","npr1")){
  for (i in 1:9267){
    ...
    try(ifelse( i == 1, write.table(mylgsnp, paste("06_GEMMAsumm/",j,"_GEMMA_top100_beta05SNP.txt"), sep = ",", col.names = TRUE), write.table(mylgsnp, paste("06_GEMMAsumm/",j,"_GEMMA_topSNP_beta05.txt"), sep = ",", col.names = FALSE, append = TRUE)))
    Sys.time()
  }
}
mytime
