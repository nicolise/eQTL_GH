...
my.gtf <- read.table("data/ensembl/B05.10/extracted/Botrytis_cinerea.ASM83294v1.38.gtf", fill=TRUE)
#these column numbers differ by gtf on Linux or laptop
num.genes <- my.gtf[unique(my.gtf$V13),]
my.gtf <- my.gtf[,c(1:8,10,13,16)]

#------------------------------------------------------------------------------------
##customize for each SNP list
#snp location:
#setwd("~/Projects/BcAt_RNAGWAS/data/GEMMA_eachAt_Bc/06_GEMMAsumm/GeneNames/")
setwd("/media/nesoltis/Soltis_AtBc_eQTL/BcAt_RNAGWAS/GEMMA_eachBc_At/05_GEMMAsumm/GeneNames")
